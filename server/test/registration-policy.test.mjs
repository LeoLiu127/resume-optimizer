...
test('a successful registration marks bootstrap as having users so later registration requires an invite', () => {
  const afterRegistration = markBootstrapAsHavingUsers({
    hasUsers: false,
    inviteOnly: true,
    minimaxConfigured: true,
  });

  assert.deepEqual(afterRegistration, {
    hasUsers: true,
    inviteOnly: true,
    minimaxConfigured: true,
  });
  assert.equal(isFirstUserRegistration(afterRegistration), false);
});
...
test('AuthGate omits the invite field and client invite validation only for the first-user policy state', () => {
  ...
  assert.match(source, /setBootstrap\(\(currentBootstrap\) => markBootstrapAsHavingUsers\(currentBootstrap\)\)/);
});
